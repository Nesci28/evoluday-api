...
export function userUpdateTest(): void {
  ...
  it("should update the specified User", async () => {
    ...
    const { one, two } = ctx.testHandler.parseCompare(ctx.userUpdate[ctx.userKey], result.value![ctx.userKey]);
  ...
  it("should update the specified User as dry-run (not saving)", async () => {
    ...
    const { one, two } = ctx.testHandler.parseCompare(updatedValue, result.value![ctx.userKey]);
    typeof one === "object" ? expect(one).toMatchObject(two) : expect(one).toEqual(two);
    expect(result.isSuccess).toEqual(true);
    expect(result.value!.id).toEqual(user.id);
  });
