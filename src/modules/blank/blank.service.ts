...
export class BlankService {
  constructor(private readonly blankRepository: BlankRepository) {}

  public async create(blank: BlankCreate, isDryRun?: boolean): Promise<Blank> {
    const res = await this.blankRepository.create(blank, isDryRun);
    return res;
  }

  public async createMany(blankBulk: BlankCreate[]): Promise<Blank[]> {
  ...
  public async getById(blankId: string, populate?: never): Promise<Blank> {
    const res = await this.blankRepository.getById(blankId, populate);
    return res;
  }

  public async patch(blankId: string, blank: BlankPatch): Promise<Blank> {
